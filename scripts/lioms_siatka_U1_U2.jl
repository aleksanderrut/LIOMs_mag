...
"""
Two-leg XXZ ladder Hamiltonian with periodic boundary conditions along the legs.
Hamiltonian written in the S+, S-, Sz basis.
"""
function XXZ_ladder(J::Float64, L::Int, Δ::Float64, U_1::Float64, U_2::Float64)
  H = ps.Operator(L * 2)
  for l in 1:L
    lp = mod1(l + 1, L)
    for y in 1:2
...
function run_grid_scan(J::Float64, L::Int, Delta::Float64, max_supp::Int, time_reversal::Symbol, parity::Symbol, conserve_Sz::Symbol, grid_U1::Int, grid_U2::Int, eig_index::Int, U1_min::Float64, U1_max::Float64, U2_min::Float64, U2_max::Float64)
  println("Running grid scan mode.")
  println("U_1 grid size = ", grid_U1)
  println("U_2 grid size = ", grid_U2)
  println("Eigenvalue index saved = ", eig_index)
  println("U_1 range = [", U1_min, ", ", U1_max, "]")
  println("U_2 range = [", U2_min, ", ", U2_max, "]")
  ...
  U1_values = collect(range(U1_min, U1_max, length = grid_U1))
  U2_values = collect(range(U2_min, U2_max, length = grid_U2))

  base_path = "$(@__DIR__)/liom_mag_dane"
  grid_path = "$(base_path)/siatka_U1_U2"
  mkpath(grid_path)

  file_tag = "grid_ladder_mag_M_$(max_supp)_J_$(J)_d_$(Delta)_T_$(time_reversal)_P_$(parity)_Sz_cons_$(conserve_Sz)_U1n_$(grid_U1)_U2n_$(grid_U2)_eig_$(eig_index)"
  filename_grid = "$(grid_path)/$(file_tag).txt"

  total_points = grid_U1 * grid_U2
  p = Progress(total_points; desc = "Computing U_1/U_2 grid...", showspeed = true)

  open(filename_grid, "w") do io
    for U1 in U1_values
      for U2 in U2_values
        H = XXZ_ladder(J, L, Delta, U1, U2)
        evals = compute_evals_from_prepared_basis(H, ops)

        lambda = evals[eig_index]

        println(io, U1, "\t", U2, "\t", lambda)
...
function parse_args()
  ...
  @add_arg_table s begin
    ...
    default = "yes"
    "--grid-U1"
    help = "Number of grid points in U_1 direction. If > 1, grid scan mode is used."
    arg_type = Int
    default = 1
    "--grid-U2"
    help = "Number of grid points in U_2 direction. If > 1, grid scan mode is used."
    arg_type = Int
    default = 1
    "--eig-index"
    ...
    default = 1
    "--U1-min"
    help = "Minimal U_1 value in grid scan"
    arg_type = Float64
    default = 0.0
    "--U1-max"
    help = "Maximal U_1 value in grid scan"
    arg_type = Float64
    default = 10.0
    "--U2-min"
    help = "Minimal U_2 value in grid scan"
    arg_type = Float64
    default = 0.0
    "--U2-max"
    help = "Maximal U_2 value in grid scan"
    arg_type = Float64
    default = 10.0
  end
...
function main()
  ...
  L = 2 * max_supp + 2
  grid_U1 = args["grid-U1"]
  grid_U2 = args["grid-U2"]
  ...
  if grid_U1 > 1 || grid_U2 > 1
    run_grid_scan(J, L, Delta, max_supp, time_reversal, parity, conserve_Sz, grid_U1, grid_U2, eig_index, U1_min, U1_max, U2_min, U2_max)
    return
  end

H = XXZ_ladder(J, L, Delta, U_1, U_2)
  ...
  mkpath(basis_path)
  mkpath(logs_path)
  file_tag = "ladder_mag_M_$(max_supp)_J_$(J)_d_$(Delta)_U_1_$(U_1)_U_2_$(U_2)_T_$(time_reversal)_P_$(parity)_Sz_cons_$(conserve_Sz)"
